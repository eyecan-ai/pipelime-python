...
class TestGeneralCommands:
    ...
    def test_remote_add(
        self, minimnist_dataset, nproc, prefetch, keys, slicing, tmp_path
    ):
        from pipelime.remotes import make_remote_url
        from pipelime.commands import AddRemoteCommand
        from pipelime.sequences import SamplesSequence
        ...
        # check output
        outseq = SamplesSequence.from_underfolder(tmp_path / "output")
        srcseq = SamplesSequence.from_underfolder(
            minimnist_dataset["path"], merge_root_items=False
        )
        remote_url = Path(remote_url._replace(netloc="").geturl())
        ...
        for i, (s, o) in enumerate(zip(srcseq, outseq)):
            for k, v in o.items():
                if i in slice_range and (keys is None or k in keys):
                    ...
                    assert Path(v.remote_sources[0].geturl()).parent == remote_url
                else:
