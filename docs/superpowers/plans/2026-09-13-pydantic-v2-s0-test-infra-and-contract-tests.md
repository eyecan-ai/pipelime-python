# S0 — Test infrastructure and contract tests (written against v1)
...
**Spec:** `docs/superpowers/specs/2026-09-12-pydantic-v2-migration-design.md` §5; master plan `2026-09-13-pydantic-v2-migration.md`.

## Global Constraints

- Do not modify anything under `pipelime/` in this subtask.
- Every contract test must pass on the current v1 code unless marked `xfail(V1, strict=True)` with a reason naming the spec section.
- Commands run from the repo root with `.venv/bin/python -m pytest`.

---

### Task S0-T1: pytest-xdist and per-file grouping

**Files:**
- Modify: `pyproject.toml` (tests extra)
- Modify: `tests/conftest.py`
- Modify: `tests/pipelime/piper/progress/listener/receiver/test_receiver_zmq.py`, `tests/pipelime/piper/progress/tracker/test_tracker_zmq.py`
- Modify: `Makefile`
...
Run: `.venv/bin/python -m pip install pytest-xdist`
...
- [ ] **Step 3: Put both ZMQ tests in one group**

In `tests/pipelime/piper/progress/listener/receiver/test_receiver_zmq.py` add `import pytest` and decorate the class:
...
In `tests/pipelime/piper/progress/tracker/test_tracker_zmq.py` decorate the class (pytest is already imported):
...
- [ ] **Step 4: Add Makefile targets** (after the existing `test:` target)
...
```bash
git add pyproject.toml tests/conftest.py Makefile tests/pipelime/piper/progress/listener/receiver/test_receiver_zmq.py tests/pipelime/piper/progress/tracker/test_tracker_zmq.py
git commit -m "test: add pytest-xdist with per-file groups and test tiers"
```

---

### Task S0-T2: Test-changes ledger

**Files:**
- Create: `tests/TEST_CHANGES.md`
...
```bash
git add tests/TEST_CHANGES.md
git commit -m "test: add TEST_CHANGES ledger"
```

---

### Task S0-T3: Contract test module skeleton and dual imports

**Files:**
- Create: `tests/pipelime/test_pydantic_contract.py`

**Interfaces:**
- Produces (module-level helpers used by all later contract tasks): `V1: bool`, `pyd` (the pydantic namespace in use), `Field`, `dump(model, **kw)`, `parse_as(tp, value)`, `make_model(name, **fields)`, `contract_identity(x)`, `contract_action(x)`, `THIS_FILE`.
...
- [ ] **Step 2: Run the (empty) module to check it imports**

Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py`
Expected: `no tests ran` (exit code 5), no import error.

- [ ] **Step 3: Commit**

```bash
git add tests/pipelime/test_pydantic_contract.py
git commit -m "test: contract test module skeleton with dual pydantic imports"
```

---

### Task S0-T4: Contracts — compact forms and union resolution
...
class TestCompactForms:
    ...
    def test_splits_union_resolution(self, tmp_path: Path):
        ...
        s = H(s=f"1,{tmp_path}").s
        assert type(s) is AbsoluteSplit and s.output.folder == tmp_path.resolve()
        s = H(s=f"0.3,{tmp_path}").s
        assert type(s) is PercSplit and s.output.folder == tmp_path.resolve()
...
Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py -k CompactForms`
Expected: 6 passed.
...
```bash
git add tests/pipelime/test_pydantic_contract.py
git commit -m "test(contract): compact forms and split union resolution"
```
...
class TestRootWrappers:
    ...
    def test_type_def_and_item_type(self):
        ...
        assert it.dict() == {"__root__": "ImageItem"}
        assert str(it) == "ImageItem"
        H = make_model("H", tp=(plt.ItemType, ...))
        assert dump(H(tp="PngImageItem")) == {"tp": "PngImageItem"}
    ...
    def test_callable_def(self):
        ...
        assert H(fn=f"{MODULE}.contract_identity").fn is contract_identity
...
- [ ] **Step 2: Run and fix the `ContractItem` skeleton if `pli.Item` needs more abstract methods** (check `pipelime/items/base.py`: the abstract methods of `Item` must all be implemented, otherwise `TypeDef` rejects nothing but the class cannot be defined).

Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py -k RootWrappers`
Expected: 5 passed.
...
```bash
git add tests/pipelime/test_pydantic_contract.py
git commit -m "test(contract): root value wrappers (__root__, envelope, nesting)"
```

---

### Task S0-T6: Contracts — v1 Optional semantics and polymorphic dumps
...
Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py -k "V1Optional or Polymorphic"`
Expected: 7 passed. If `test_entity`'s dump assertion fails on v1 because `dump()` returns the raw item under a different key, print `dump(e)` and adjust the expected dict to what v1 produces — that value *is* the contract.

- [ ] **Step 3: Commit**

```bash
git add tests/pipelime/test_pydantic_contract.py
git commit -m "test(contract): v1 Optional semantics and polymorphic nested dumps"
```

---

### Task S0-T7: Contracts — command framework
...
Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py -k CommandFramework`
Expected: 10 passed, 1 xfailed. If `test_nodes_definition_dump_and_validate` fails on the `by_alias` line, replace the `assert dump(nodes, by_alias=True)[...]` line with `assert "__root__" in nodes.dict(by_alias=True)`.

- [ ] **Step 3: Commit**

```bash
git add tests/pipelime/test_pydantic_contract.py
git commit -m "test(contract): command framework, @command, LazyCommand, NodesDefinition"
```

---

### Task S0-T8: Contracts — stages, entities, sequences
    ...
    return pls.Sample(
        {
            ...
            "other": pli.NumpyItem(np.array([9])),
        }
    )
...
- [ ] **Step 2: Run and adjust the pinned dumps to v1's actual output where the plan guessed** (`test_entity_parsing_and_merge` and `test_stage_entity_dump_roundtrip`): print the value on failure, verify it is sensible, paste it as the expected value.

Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py -k "StagesAndEntities or TestSequences"`
...
```bash
git add tests/pipelime/test_pydantic_contract.py
git commit -m "test(contract): stages, entities, sequences, pickling"
```

---

### Task S0-T9: Contracts — validation interfaces, validator ordering, error formatting, help rendering

**Files:**
- Modify: `tests/pipelime/test_pydantic_contract.py` (append)
- Create: `tests/sample_data/contract/help_contract_ports.txt` (snapshot)
...
- [ ] **Step 2: Run** (the first run creates the snapshot; run twice)

Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py -k "ValidationInterfaces or HelpRendering"` (twice)
Expected: 8 passed both times; `tests/sample_data/contract/help_contract_ports.txt` exists. If `pp.rprint` is not the name used inside `pretty_print.py` (check `grep -n "rprint\|from rich import print" pipelime/cli/pretty_print.py`), patch the name that module actually calls.

- [ ] **Step 3: Commit**

```bash
git add tests/pipelime/test_pydantic_contract.py tests/sample_data/contract/help_contract_ports.txt
git commit -m "test(contract): validation interfaces, validator ordering, error text, help snapshot"
```

---

### Task S0-T10: Contracts — modern type hints
...
Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py -k ModernTypeHints`
Expected: 3 passed, 1 xfailed.

- [ ] **Step 3: Run the whole contract module and Tier 0**

Run: `.venv/bin/python -m pytest -q -o addopts="" tests/pipelime/test_pydantic_contract.py`
Expected: all passed except 3 xfailed (`var_keyword_expansion`, `to_pipe_roundtrip`, `test_tui`).
Run: `make test-tier0`
Expected: green.

- [ ] **Step 4: Commit and update the ledger**

```bash
git add tests/pipelime/test_pydantic_contract.py
git commit -m "test(contract): modern type hints (validation, help, TUI xfail, DAG)"
```

Record in `docs/superpowers/plans/2026-09-13-pydantic-v2-migration-progress.md`: S0 done, gate `make test-tier0` + `make test-full` results, commit hashes, the three expected xfails.

Then open `2026-09-13-pydantic-v2-s1-toolkit-and-pydantic-types.md`.
